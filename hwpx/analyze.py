"""
hwpx/analyze.py — A 양식 분석 (MVP: 표 셀만).

계약: docs/contracts/engine.md §2
"""
import hashlib
import re
import uuid
...
_RULE4_LABEL_PATTERNS = sorted(
    [(re.escape(label), label) for label in _RULE4_LABELS],
...
def analyze_a(path: str) -> dict:
    ...
    for sec_path in section_paths:
        try:
            ...
            warnings.append(f"섹션 없음: {sec_path}")
            continue
        try:
            root = etree.fromstring(xml_bytes)
        except Exception as exc:
            warnings.append(f"섹션 파싱 실패: {sec_path} ({exc})")
            continue
        ...
        for t_idx, tbl in enumerate(tables):
            ...
            for (col, row), tc in sorted(cell_map.items()):
                ...
                if col + 1 > max_col:
                    continue
                right_tc = cell_map.get((col + 1, row))
                if right_tc is None:
                    continue
